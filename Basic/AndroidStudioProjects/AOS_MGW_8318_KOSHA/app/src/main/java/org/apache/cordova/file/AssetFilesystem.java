...
public class AssetFilesystem extends Filesystem {
	...
	// A custom gradle hook creates the cdvasset.manifest file, which speeds up
	// asset listing a tonne.
	// See:
	// http://stackoverflow.com/questions/16911558/android-assetmanager-list-incredibly-slow
		...
		synchronized (listCacheLock) {
			if (listCache == null) {
				...
				try {
					...
					// Asset manifest won't exist if the gradle hook isn't set
					// up correctly.
					Debug.trace(e.getMessage());
				} finally {
					if (ois != null) {
	...
	private long getAssetSize(String assetPath) throws FileNotFoundException {
		...
		try {
			...
			if (length < 0) {
				// available() doesn't always yield the file size, but for
				// assets it does.
	...
	public LocalFilesystemURL toLocalUri(Uri inputURL) {
		...
		Uri.Builder b = new Uri.Builder().scheme(LocalFilesystemURL.FILESYSTEM_PROTOCOL).authority("localhost").path(name);
		if (!subPath.isEmpty()) {
	...
	@Override
	public JSONObject getFileForLocalURL(LocalFilesystemURL inputURL, String path, JSONObject options, boolean directory)
			throws FileExistsException, FileNotFoundException, IOException, TypeMismatchException, EncodingException, JSONException {
		...
		if(requestedURL ==null) return null;
